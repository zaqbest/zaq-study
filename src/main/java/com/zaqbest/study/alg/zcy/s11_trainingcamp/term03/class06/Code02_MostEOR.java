...
/**
 * 给定一个数组，可以划分成多个数组；要求这些数组中，有尽可能多的异或和为0的数组，返回异或和为0的数组，最多有多少个
 */
public class Code02_MostEOR {
	...
	public static int mostEOR(int[] arr) {
		...
		int N = arr.length;
		//dp[i]->arr[0..i]在最优划分下，异或和为0最多部分有多少个
		int[] dp = new int[N]; // dp[i] = 0
		//key:某个前缀异或和
		//value: 这个前缀异或和出现的最晚位置（0~value)
		...
		for (int i = 0; i < arr.length; i++) {//i所在的最后一块
			...
			//前缀异或和出现的最晚结尾位置，k+1是离i最近的
			if (map.containsKey(sum)) {//上一次这个异或和出现的位置
				//pre -> pre+1 -> i,最优划分，最后一个部分的开始位置
				//(pre+1,i)最后一个部分
				int pre = map.get(sum);
				dp[i] = pre == -1 ? 1 : (dp[pre] + 1);//sum 0..pre(pre+1..i)
			}
