package com.zaqbest.study.alg.zcy.s11_trainingcamp.term05.class03;

import java.util.Scanner;

/**
 * 牛牛和15个朋友来玩打土豪分田地的游戏，牛牛决定让你来分田地，地主的田地可以看成是一个矩形，
 * 每个位置有一个价值。分割田地的方法是横竖各切三刀，分成 16 份，作为领导干部，牛牛总是会选择其中总 价值最小的一份田地，
 * 作为牛牛最好的朋友，你希望牛牛取得的田地的价值和尽可能大，你知道这个值最大 可以是多少吗?
 *
 * 输入描述:
 * 每个输入包含 1 个测试用例。每个测试用例的第一行包含两个整数 n 和 m(1 <= n, m <= 75)，表示田地 的大小，
...
public class Code01_NiuNiuSplitField {
	...
	//
	// public static int[] getSplitDpArray(int[] arr) {
	// int[] dp = new int[arr.length];
	// int split = 0;
	// dp[1] = Math.min(arr[0], arr[1]);
	// for (int i = 2; i < arr.length; i++) {
	// int minsMax = Math.min(getSum(arr,0, split), getSum(arr,split+1,i));
	// while (split < i - 1) {
	// int nextMin = Math.min(getSum(arr,0, split+1), getSum(arr,split+2,i));
	// if (nextMin < minsMax) {
	// break;
	// } else {
	// minsMax = nextMin;
